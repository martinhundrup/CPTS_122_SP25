...
void ListApp::runApp(void)
{
	...
	mOutputStream << "mean score: " << findMean() << std::endl;
	mOutputStream << "max score: " << mList.deleteAtEnd() << std::endl;
	mOutputStream << "min score: " << mList.deleteAtFront();
}

double ListApp::findMean(void) {

	int sum = 0;
	int count = 0;
	
	ListNode* pCur = mList.getHeadPtr();

	while (pCur != nullptr) {

		sum += pCur->getData(); // Add to rolling sum.
		count++; // Increment count.

		pCur = pCur->getNextPtr(); // Update pCur for next iteration.
	}
...
ifstream& operator>> (ifstream& input, List& rhs)
{
	...
	while (!input.eof()) // read all lines from the file, populate the list with the scores only!
	{
		...
		rhs.insertInOrder(score); // changed to insert in order for easily finding the min and max.
	}
